      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button type="text"
            ...
            icon="icon-view"
            size="small">查看预计认定</el-button>
          <el-button type="text"
            v-if="scope.row.status == '1' || scope.row.status == '2' || scope.row.status == '3'"
            size="small">预警认定</el-button>
          <el-button type="text"
            icon="icon-view"
            size="small">查看贷后报告</el-button>
...
import { getWarningCognizance } from '@/api/bussiness/post-loan-manage/index'
...
export default {
  ...
  created() {
    this._getWarningCognizance()
  },
  mounted() {},
  methods: {
    _getWarningCognizance() {
      getWarningCognizance(this.searchData).then(res => {
